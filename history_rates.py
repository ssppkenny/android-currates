...
class FxtopRate:
    ...
    def plot(self):
        ...
        axis.set_title(f"Exchange rates on {self._date_str} from {self._from_cur} to {self._to_cur}" )
        axis.set_xlabel("Date")
        axis.set_ylabel("Exchange Rate")
        data_frame = df.sort_index(axis=0 ,ascending=False)
        x = range(len(data_frame[1]))
        y = df[1]
        coef = np.polyfit(x,y,1)
        poly1d_fn = np.poly1d(coef) 
        axis.plot(df[0], y, '-', df[0], poly1d_fn(x), '-')
        return fig
